export class UnicodeSteganography {
  private chars: string[] = [];
  private radix = 0;
  private codelengthText = 0;
  private codelengthBinary = 0;
  public static splitPattern =
  ...
  public static getInstance() {
    return new UnicodeSteganography();
  }
  ...
  /**
    ...
    return: null
   */
  public setUseChars(newchars: string) {
  ...
  public encodeText(text1: string, text2: string): any {
    return this.combine_shuffle_string(
      text1,
      this.encode_to_zerowidth_characters_text(text2),
      this.codelengthText
    );
  }
   ...
   */
  public encodeBinary(text: string, data: any): any {
    return this.combine_shuffle_string(
      text,
      this.encode_to_zerowidth_characters_binary(data),
   ...
   */
  public decodeText(text: string): any {
    var splitted = this.split_zerowidth_characters(text);

    return {
      originalText: splitted.originalText,
      hiddenText: this.decode_from_zero_width_characters_text(
        splitted.hiddenText
      ),
    };
  }

  public sensitiveFilter(text: string): string {
    let filterAfter = "";
    for (const i of text) {
      let idx =
        Math.floor(Math.random() * this.chars.length) % this.chars.length;
      filterAfter += i + this.chars[idx];
    }
    return filterAfter;
  }
   ...
   */
  private decodeBinary(text: string): any {
    var splitted = this.split_zerowidth_characters(text);

    return {
      originalText: splitted.originalText,
      hiddenData: this.decode_from_zero_width_characters_binary(
        splitted.hiddenText
      ),
    };
  }

  private encode_to_zerowidth_characters_text(str1: string): any {
    ...
    for (i = 0; i < this.radix; i++) {
      r = r.replace(new RegExp(i.toFixed(), "g"), this.chars[i]);
    }

    return r;
  }

  private encode_to_zerowidth_characters_binary(u8ary: any): any {
  ...
  private combine_shuffle_string(
    str1: string,
    str2: string,
    codelength: number
  ): string {
  ...
  private split_zerowidth_characters(str1: string): any {
    var result = {
      originalText: "",
      hiddenText: "",
    };
  ...
  private decode_from_zero_width_characters_text(str1: string): any {
    var r = str1;
    var i;
    var result = [];
    for (i = 0; i < this.radix; i++) {
      r = r.replace(new RegExp(this.chars[i], "g"), i.toFixed());
    }
  ...
  private decode_from_zero_width_characters_binary(str1: string): any {
    var r = str1;
    var i;
    var j;
    var result = new Uint8Array(Math.ceil(str1.length / this.codelengthBinary));
